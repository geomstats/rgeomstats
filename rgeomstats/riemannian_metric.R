...
loss <- function(y.pred, y.true, metric){
  # Loss function given by a riemannian metric,
  # expressed as the squared geodesic distance between the prediction
  # and the ground truth.

  loss = metric$squared_dist(y.pred, y.true)
  return(loss)
}

grad <- function(y.pred, y.true, metric){
  # Closed-form for the gradient of the loss function.

  tangent.vec <- metric$log(base_point=y.pred, point=y.true)
  grad.vec <- -2 * tangent.vec

  inner.prod.mat <- metric$inner.product.matric(base.point=y.pred)

  grad <- gs$backend$dot(grad.vec,  gs$backend$transpose(inner.prod.mat, axes=c(0,2,1)))
  return(grad)
}
...
RiemannianMetric <- setRefClass("RiemannianMetric",
                              fields = "dimension",
                              methods = list(
                                initialize(dimension, signature=none){
                                  stopifnot(dimension %% 1 == 0)
                                  stopifnot(dimension > 0)
                                  .self$dimension <- dimension
                                  .self$signature <- signature
                                },
                                InnerProduct(tangent.vec.a, tangent.vec.b, base_point=None){
                                  "Inner product between two tangent vectors at a base point."
                                  tangent.vec.a <- to_ndarray(tangent_vec_a, to_ndim=2)
                                  tangent.vec.b <- to_ndarray(tangent_vec_b, to_ndim=2)
                                  n.tangent.vec.a <- gs$backend$shape(tangent.vec.a)[1]
                                  n.tangent.vec.b <- gs$backend$shape(tangent.vec.b)[1]

                                  inner.prod.mat <- .self$inner.product.matrix(base_point)
                                  inner.prod.mat <- tondarray(inner.prod.mat, to_ndim=3)
                                  n_mats <- gs$backend$shape(inner.prod.mat)[1]
                                  ...
                                  n.tiles.a <- gs$backend$cast(n.tiles.a, gs$backend$int32) #what??
                                  tangent.vec.a <- gs$backend$tile(tangent.vec.b, c(n.tiles.a, 1))

                                  n.tiles.b <- n.inner.prod / n.tangent.vec.b
                                  n.tiles.b <- gs$backend$cast(n.tiles.b, gs$backend$int32) #what??
                                  tangent.vec.b <- gs$backend$tile(tangent.vec.b, c(n.tiles.b, 1))

                                  n.tiles.mat <- n.inner.prod / n.mats
                                  n.tiles.mat <- gs$backend$cast(n.tiles.mat, gs$backend$int32)
                                  inner.prod.mat <- gs$backend$tile(inner.prod.mat, c(n.tiles.mat, 1, 1))

                                  aux <- gs$backend$einsum('nj,njk->nk', tangent.vec.a, inner.prod.mat)
                                  inner.prod <- gs$backend$einsum('nk,nk->n', aux, tangent.vec.b)
                                  inner.prod <- tondarray(inner.prod, to_ndim=2, axis=1)

                                  stopifnot(gs$backend$ndim(inner.prod) == 2 )
                                  return(inner.prod)
